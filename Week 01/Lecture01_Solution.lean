...
-- Lecture 01: Basics of Lean — Solutions
-- © Jaume de Dios 2026
...
example
  ...
  intro g
  calc
    g^6 = (g^3)^2 := by exact pow_mul g 3 2
    _ = (1)^2 := by rw [ord3 g]
    _ = 1 := by rw [one_pow]
...
theorem continuous_at_comp {f g : ℝ → ℝ} {x : ℝ}
    ...
    continuous_at (g ∘ f) x := by
  unfold continuous_at at hf hg ⊢
  intro ε hε
  obtain ⟨δg, hδg, hgδ⟩ := hg ε hε
  obtain ⟨δf, hδf, hfδ⟩ := hf δg hδg
  use δf
  constructor
  · exact hδf
  · intro y hy
